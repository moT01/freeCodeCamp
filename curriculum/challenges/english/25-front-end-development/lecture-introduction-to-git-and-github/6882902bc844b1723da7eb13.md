...
For Linux systems, Git often comes preinstalled with most distros. If you do not have Git pre-installed, you should be able to install it with your package manager such as sudo apt-get install git or sudo pacman -S git.
...
To make sure the installation worked, run the git --version command in the terminal.
...
Configuring your user name and email is important because Git will use this information for every commit you make in a project. A commit is like a snapshot in the timeline of a project and you will learn more about that in a future lecture.
...
To see your list of configurations so far, you can run the following command:

git config --list

Now that you have Git installed and customized, you are now ready to start adding version control to your projects.
...
Refer back to the section talking about git config for the answer.

---

Shows a list of only color options in your git config.
